...
FROM storestock
    ...
    WHERE storestock.quantity > 0
        AND storestock.quantity - coalesce(old.quantity, 0) > @significantVolumeChange
...
FROM storestock
    ...
    LEFT JOIN store
        ON storestock.siteid = store.storeidasinteger
    WHERE storestock.quantity > 0
        AND (storestock.quantity - coalesce(old.quantity, 0)) * article.price > @significantValueChange
        AND coalesce(old.quantity, 0) = 0
        -- optionally more conditions here
